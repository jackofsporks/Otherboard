...
keyFuncs.triggerSnippet = function() {
  ...
  const alignWhenRendered = function(acp, listElm, numTries){
    numTries++
    if (numTries > 100) return
    const marginLeft = parseInt(window.getComputedStyle(listElm)["margin-left"])
    console.log("why?!?! -->", numTries, marginLeft, acp.getBoundingClientRect().left);
    if (marginLeft !== 0) {
      let left = acp.getBoundingClientRect().left
      left += -1 * marginLeft
      acp.style.left = left + "px"
      console.log("where my friends at? -->", numTries, marginLeft, acp.getBoundingClientRect().left);
    } else setTimeout(function(){alignWhenRendered(acp, listElm, numTries)}, 5)
  }

  const move = function(numTries){
    numTries++
    ...
    if (acp) {
      ...
      alignWhenRendered(acp, listElm, 0)
    } else if (numTries > 100 && key.options) key.options.show(false)
    else setTimeout(function(){move(numTries)}, 1)
  } // timing again!! Booo!!! Can break with lag

  move(0)
  setTimeout(function(){move(90)}, 500)
