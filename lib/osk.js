...
export default {
  ...
  activate(state) {
    ...
    if (state.history && !reset && !atom.config.get("osk.debug.resetToDefaultsOnStartup")) {osk = this.osk = atom.deserializers.deserialize(state); console.log(osk, atom.deserializers.deserialize)}
  ...
  deserializeOSK(deserializationObject) {
    console.log(deserializationObject.data);
