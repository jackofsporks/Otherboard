...
export default assignParentProps = function (caller, osk, config, props, ChildConstructor) {
  ...
  if (props.includes("addChildren")) {
    caller.addChildren = (function () {
      const parent = this
      const childrenData = parent.config.children
  ...
  if (props.includes("add")) {
    caller.add = (function (childConfig) {
      // TODO: Set off event when child added so that view can add it to the DOM?
      const parent = this
      const child = new parent.ChildConstructor(osk, parent, childConfig)
