...
export default class ConfigOptionsView {
  ...
  show() {
    ...
    const togglers = document.querySelectorAll(".osk-config-toggle")
    for (let tgi = 0; tgi < togglers.length; ++tgi) {
      togglers[tgi].classList.add("osk-config-open")
    }
  ...
  hide() {
    ...
    const togglers = document.querySelectorAll(".osk-config-toggle")
    for (let tgi = 0; tgi < togglers.length; ++tgi) {
      togglers[tgi].classList.remove("osk-config-open")
    }
