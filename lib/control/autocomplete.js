...
export default class AutocompleteThief {
  ...
  constructor(osk) {
    ...
    // NOTE/TODO Use our own version of autocomplete plus with node modules so it doesn't break unexpectedly
    // TODO control "consume suggestion text following the cursor" autocomplete-plus option to not consume?!?!
    // TODO If the cursor scope doesn't match the panel scope don't do autocomplete stuffs
    const aThief = this
    contextThief = this
  ...
  wait(indexReplacement) {
    ...
    if (selectedElement) {
      ...
      if (typeof indexReplacement === "number") i = indexReplacement
      ...
      if (aThief.index !== i) {
        ...
        console.log(listElement.querySelector(".selected"))
        // setTimeout(function(){
          ...
          console.log("elements? -->", elements, aThief.index, toSelect, toClick);
          // NEXT check if click is working
          ...
          // aThief.confirming = true // confirming manually, so don't actually release key
          // // NOTE Without confirming "activate" triggers this whole "wait" thing again
  ...
  clickCancel(e) {
    ...
    if (!fromList && aThief.osk.activeKey.view.node !== e.target) aThief.release(e, "cancel-click")
    ...
    // if (!fromList && !/osk-key/.test(targetClasses)) aThief.release(e, "cancel-click")
  }
  ...
  subscribe() {
    ...
    // ## Show list
    // clicking a key?!? (got that one goin' on, oh yeah) hovering over a key instead?!?!
    // tapping on a key
    ...
    // TODO hovering navigation over a list item (also select it then)
    ...
    // QUESTION if user stops hovering does the list disappear/cancel?!?
    ...
    subs.add(listObj.onDidCancel(function(e){aThief.release(e, "cancel-autocomplete-plus")}))
    // √ minimizing osk
    subs.add(aThief.osk.emitter.on("minimized", function minimizeRelease(caller){aThief.release(null, "cancel-"+caller.id)}))
  ...
  mouseEnterLIHandler(e) {
    ...
    console.log("mouseentered index -->", i);
    aThief.wait(i)

    // const editorElement = atom.views.getView(aThief.editor)
    // const toClick = this.querySelector("span.word")
    //
    // if (!aThief.firstHitOnThisKey) {
    //   // Reverting destroys checkpoint
    //   aThief.buffer.revertToCheckpoint(aThief.osk.postInsertionCheckpoint)
    //   aThief.osk.postInsertionCheckpoint = aThief.buffer.createCheckpoint()
    //   // TODO only show board-context appropriate completions
    //   atom.commands.dispatch(editorElement, "autocomplete-plus:activate")
    // }
    //
    ...
    // aThief.confirming = true // confirming manually, so don't actually release key
    // atom.commands.dispatch(editorElement, "autocomplete-plus:confirm") // <-- async?!?!
    // console.log("selected with mouseenter. this, target-->", this, e.target, this.querySelector("span.word"));
  }
  ...
  testKeydown(e){
    ...
    else return aThief.release(e, "confirm")
  }
  ...
  destroy() {
    const aThief = contextThief
    aThief.osk.activeKey.deactivate() // Destroy this autocomplete
  }
