...
export default class AutocompleteThief {
  ...
  constructor(osk) {
    ...
    // ### Non-inserters
    // capslock, shift, ctrl, alt, cmd, right, left, up, down, meta, (altgr, right-alt, alt-gr, alt-graph?!?), (option?!? windows?!?), (page-up/down/home/end?!), numlock, scrolllock, escape, menu?!?
    // ### ignored inserters
    ...
    // √ clicking elsewhere on the editor (any click anywhere other than suggestionList or the key, which confirm, TODO or inside the snippet!!!! or on another osk key (that would be a confirmation)!!!!!). Assigned in dom event listener anytime list appears from an osk click
  ...
  testKeydown(e){
    ...
    console.log(keyAtomName, aThief.confirming)
    if (aThief.ignoredKeys.indexOf(keyAtomName) > -1) return false
  ...
  preFinishReset() {
    ...
    aThief.alreadyReleased = false
    aThief.firstHitOnThisKey = false
    document.body.removeEventListener("keydown", aThief.testKeydown)
  }
