...
export default class AutocompleteThief {
  constructor(osk, key) {
    ...
    if (!osk.modals) {osk.modals = {}}
    if (!osk.modals.completions) {
      osk.modals.completions = document.createElement("osk-completions")
    }
    ...
    aThief.functs = {
      hide: {
        caller: aThief.autocompletePlusManager.suggestionList,
        original: aThief.autocompletePlusManager.suggestionList.hide,
        override: function(){console.log("destroyed!!!");}},
      show: {caller: aThief.autocompletePlusManager.suggestionList,
        original: aThief.autocompletePlusManager.suggestionList.show,
        override: aThief.showAtKey.bind(aThief.autocompletePlusManager.suggestionList)},
      hideSuggestionList: {caller: aThief.autocompletePlusManager,
        original: aThief.autocompletePlusManager.hideSuggestionList,
        override: function(){}},
    }
  ...
  showAtKey(editor, options){
    console.log("this in new show funct -->");
    const list = this
    const aThief = contextThief
    // if it already exists move it
    if (list.activeEditor === editor && list.suggestionMarker) {
      // TODO
    } else {
      list.destroyOverlay()
      if (editor) {
        // We gotz to here all the way in now just make it visibile
        let comps = aThief.osk.modals.completions
        comps.appendChild(list.suggestionListElement.element)
        comps.classList.add("autocomplete-plus")
        comps.style.top = "0px"
        aThief.osk.active.panel.view.node.classList.add("autocomplete-active")
        console.log(comps);

        // list.activeEditor = editor
        // const editorElement = atom.views.getView(list.activeEditor)
        // if (editorElement && editorElement.classList) {
        //   editorElement.classList.add('autocomplete-active')
        // }
        // list.addBindings(editor)
      }
    }
    return list
  } // showAtKey
  ...
  //
  //       let placeMarker (editor, marker, markerOptions) {
  ...
  //         }
  //
  //         this.overlayDecoration = editor.decorateMarker(marker, markerOptions)
  //         this.addBindings(editor)
  //         return this
  //       }
  //
  //       this.placeMarker(editor, marker, {type: 'overlay', item: this._suggestionListElement, position: 'tail', class: 'autocomplete-plus'})
  //
  //       return this
  //     }
  //   }
  ...
  restoreFuncts() {
    ...
    //     let functGroup = aThief.functs[name]
    //     functGroup.caller[name] = functGroup.original
    //   }
    // }
  } // restoreFuncts
  ...
  destroyFuncts() {
    ...
    for (let name in aThief.functs) {
      if (aThief.functs.hasOwnProperty(name)) {
        let functGroup = aThief.functs[name]
        functGroup.caller[name] = functGroup.override
      }
    }
  }
  ...
  wait(clickSelected) {
    ...
    aThief.osk.active.autocomplete = aThief
