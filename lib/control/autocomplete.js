...
export default class AutocompleteThief {
  constructor(osk, key) {
    // holding down or in a comment or w/e
    // BUG if undo while still in autocomplete mode undo goes back TWO steps. This is bad.
    // BUG if no autocomplete list can't esc to cancel
    ...
    // ~~BUG Pretty Urgent. Sticky keys leave a letter behind if no autocomplete.~~ How/where did I do this!?!!?
    ...
    aThief.subscribe()
    // "subscribeToLImice" runs when elements exist
  } // constructor
  ...
  wait(clickSelected) {
    ...
    let doWait = false
    if (selectedElement) {
      // if sticky keys then send list item objects with displayName val and realBoard funct

      if (aThief.firstHitOnThisKey) aThief.subscribeToLImice(listElement)
  ...
  subscribeToLImice(parentElm) {
    const aThief = contextThief
    const all = parentElm.querySelectorAll("li")
    for (let li = 0; li < all.length; li++) {
      let elm, disposable
      elm = all[li]
      // console.log("list item -->", elm.querySelector(".word").innerHTML);
  ...
  getStickyObjects(parentElm) {
    ...
    //   let elm, disposable
    //   elm = all[li]
    //   textElm = elm.querySelector("")
    //   // get element and text of autocompletion display thing
    //   const member = {li: elm, textElm: textElm, displayName: textElm.innerHTML, realBoard: aThief.realBoard}
    // }
    // return boardMembers
  } // getStickyObjects
  ...
  deStyleTrigger(data){
    // data.textElm.innerHTML = data.displayName
  }

  realBoard(data){
    // const aThief = contextThief
    // const liObj = data.owner
    //
    // const handler = function(e){
    //   if (e.key === liObj.letter) {
  ...
  maybeRelease(e){
    ...
    if (aThief.listObj._suggestionListElement) listElm = aThief.listObj._suggestionListElement.element
    if (!listElm) return
    // test if mouse is in list
    const rectL = listElm.getBoundingClientRect()
    let outside = false
    if (y < rectL.offsetY || x < rectL.offsetX || y > (rectL.offsetY + rectL.height) || x > (rectL.offsetX + rectL.width)){
      outside = true
    }
    console.log("maybe releasing now!", outside);
    if (!outside) return
    // get position of key
    const key = aTheif.key
    // same dang thing
    const rectK = key.view.node.getBoundingClientRect()
    if (y < rectK.offsetY || x < rectK.offsetX || y > (rectK.offsetY + rectK.height) || x > (rectK.offsetX + rectK.width)){
      outside = true
    }
    if (!outside) return
    else aThief.release("cancel-dom", e)
  }
  ...
  mouseLeaveHandler(e){
    ...
    setTimeout(function(){aThief.maybeRelease(e)}, 500)
  }
  ...
  recordValidElement(e) {
    ...
    console.log("missing ol? -->", aThief.listObj._suggestionListElement, listElm);
  }

  mouseEnterLIHandler(e) {
    const aThief = contextThief
    console.log("entering li");
    const i = parseInt(this.getAttribute("data-index"))
    aThief.clickItem(i)
  }  // li handler
  ...
  clickItem(i){
    const aThief = contextThief
    // TODO when refactoring maybe click in here instead of flying all jello all over the place
    const listElement = document.querySelector("autocomplete-suggestion-list")
    const all = listElement.querySelectorAll("li")
    for (let li = 0; li < all.length; li++) {
  ...
  cancel(releaseOp, editorView, e) {
    ...
    } catch(err) {console.log(aThief.autocompletePlusManager, err)}
  ...
  release(releaseOp, e) {
    ...
    const aThief = contextThief
    console.trace("realse the beast! -->", releaseOp, e)
