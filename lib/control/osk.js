...
import RealBoardManager from "./features/real-board-manager"
...
export default class OSK {
  // TODO keep track of the active editor
  ...
  deactivate() {
    if (this.activeKey) this.activeKey.release("cancel-osk", null)
    this.emitter.clear()
  ...
  close() {
    ...
    // this.hide()
    // No animations
    if (osk.realBoard) osk.realBoard.manager.destroy()
    if (osk.acitveKey) osk.activeKey.cancel("osk-cancel")
  ...
  switchToPanel(path, wasFromHistory) {
    ...
    const worked = osk.history.handlePanelOpened(path, wasFromHistory)
    // QUESTION does it matter if it's the same panel and nothing new is opened???
    if (worked) {
      const panel = findPathTarget(path, osk.active.panel)
      if (osk.acitveKey) osk.activeKey.cancel("osk-cancel")
      if (osk.realBoard) osk.realBoard.manager.loadKeys(panel.children)
    }
    return osk
  }
  ...
  handleRealBoard(e){
    // event is undefined
    let osk = this
    if (osk.realBoard) osk.realBoard.manager.handleType(osk, "sticky")
    else new RealBoardManager(osk, "sticky")
  }
